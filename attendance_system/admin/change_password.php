...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!password_verify($current, $user['password'])) {
        ...
        logActivity($uid, 'CHANGE_PASSWORD', 'Admin changed their password');
    }
...
$sidebarItems = [
    ...
    ['url'=>'admin/change_password.php','icon'=>'fas fa-lock',              'label'=>'Change Password', 'active'=>true],
];
...
function checkMatch() {
    ...
    const req = document.getElementById('req-diff');
    const cur = document.getElementById('currentPwd').value;

    req.classList.toggle('met', nv.length > 0 && nv !== cur);

    if (!cv) { msg.textContent = ''; return; }
    if (nv === cv) {
        msg.innerHTML = '<span style="color:#10b981"><i class="fas fa-check-circle"></i> Passwords match</span>';
    } else {
        msg.innerHTML = '<span style="color:#ef4444"><i class="fas fa-times-circle"></i> Passwords do not match</span>';
    }
}
</script>
